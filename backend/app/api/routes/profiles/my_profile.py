...
# TODO: Create a response model for completion status.
@router.get("/completion-status", response_model=dict)
def get_completion_status(session: SessionDep, current_user: CurrentUser):
    ...
    if not profile.social_links or not (
        profile.social_links.get("linkedin") or
    ...
    completed_count = 8 - len(missing_fields)
    completion_percentage = int((completed_count / 8) * 100)
