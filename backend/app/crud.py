...
def create_board_from_llm(
    ...
    # Map status to position for easy ordering
    status_to_position = {status: idx for idx, (_, status) in enumerate(default_definitions)}

    # Build a status → ai_list map from the LLM output
    ai_lists_by_status = {
        CardStatus(lst["status"]): lst
        for lst in llm_data.get("lists", [])
        if "status" in lst
    }
    ...
    for title, status in default_definitions:
        ai_list = ai_lists_by_status.get(status, {})
        board_list = BoardList(
            title=ai_list.get("title", title),
            status=status,
            position=status_to_position[status],
            board_id=board.id,
            is_llm_generated=True
        )
        session.add(board_list)
        session.flush()  # Get autogenerated ID without full commit
        status_to_list_id[status] = board_list.id

    session.commit()

    # 3. Create cards for each list (if present)
    for list_data in llm_data.get("lists", []):
        try:
            status = CardStatus(list_data["status"])
            list_id = status_to_list_id.get(status)
            if not list_id:
                continue
